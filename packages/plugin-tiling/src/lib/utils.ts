...
export function calculateTilesForPage(
  ...
  const endCol   = Math.min(maxCol,
                     Math.floor((visRight  - 1) / step) + extraRings);
  const startRow = Math.max(0, Math.floor(visTop    / step) - extraRings);
  const endRow   = Math.min(maxRow,
                     Math.floor((visBottom - 1) / step) + extraRings);
  ...
  for (let col = startCol; col <= endCol; col++) {
    ...
    for (let row = startRow; row <= endRow; row++) {
      ...
      tiles.push({
        ...
        screenRect: { origin: { x: xScreen, y: yScreen }, size: { width: wScreen, height: hScreen } },
        status: 'queued',
        srcScale: scale,
        isFallback: false,
      });
