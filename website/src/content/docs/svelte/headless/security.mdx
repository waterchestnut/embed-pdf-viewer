...
description: Understand how to handle PDF security, encryption, and permission overrides in your Svelte application.
searchable: true
---
...
<EmbedPDF engine={engine} {config}>...</EmbedPDF>
```
...
<script lang="ts">
  ...
  const perms = useDocumentPermissions(() => documentId);
</script>

<button 
  disabled={!perms.canPrint}
  style:opacity={perms.canPrint ? 1 : 0.5}
>
  Print Document
</button>
```
...
<CodeExample 
  codePaths={[
    "../../examples/svelte-tailwind/src/examples/headless/security-example.svelte"
  ]}
>
  <SecurityExample />
</CodeExample>
