...
The plugin provides the `<LayoutAnalysisLayer />` component for displaying results, the `useLayoutAnalysis` store for document-scoped reactive state, and the `useLayoutAnalysisCapability` store for direct API access.

### The `useLayoutAnalysis` Store

The `useLayoutAnalysis(getDocumentId)` store is the primary way to interact with the plugin from Svelte. It returns reactive state and scoped methods for a specific document.

```svelte
<script lang="ts">
import { useLayoutAnalysis } from '@embedpdf/plugin-layout-analysis/svelte'

let { documentId }: { documentId: string } = $props()
const la = useLayoutAnalysis(() => documentId)

const isOverlayVisible = $derived(la.layoutOverlayVisible)
</script>

<button onclick={() => la.provides?.setLayoutOverlayVisible(!la.layoutOverlayVisible)}>
  Toggle Layout Overlay
</button>
<button onclick={() => la.provides?.setTableStructureEnabled(!la.tableStructureEnabled)}>
...
function handleAnalyze() {
  ...
  const task = la.scope.analyzeAllPages()

  task.onProgress((p) => {
    if (p.stage === 'downloading-model') {
      status = `Downloading model: ${((p.loaded / p.total) * 100).toFixed(0)}%`
    } else if (p.stage === 'creating-session') {
      status = 'Initializing model...'
    } else if (p.stage === 'page-complete') {
      status = `Page ${p.completed}/${p.total} complete`
    } else {
      status = p.stage
    }
  })
  ...
  task.wait(
    (result) => {
      ...
      status = `Done — ${totalBlocks} blocks detected`
    },
    (error) => {
      status = error.type === 'abort' ? 'Cancelled' : error.reason.message
    },
  )
}
</script>

<button onclick={handleAnalyze}>{status}</button>
```
...
```svelte
<script lang="ts">
import { LayoutAnalysisLayer } from '@embedpdf/plugin-layout-analysis/svelte'
...
import { LayoutAnalysisExample } from '../../code-examples/headless/layout-analysis-example';

<CodeExample 
  codePaths={[
    "../../examples/svelte-tailwind/src/examples/headless/layout-analysis-example.svelte",
    "../../examples/svelte-tailwind/src/examples/headless/layout-analysis-example-content.svelte"
  ]}
>
  <LayoutAnalysisExample />
</CodeExample>
...
| **`class`** | `string` | Optional CSS class applied to the container element. |
| **`style`** | `string` | Optional inline styles applied to the container element. |

### Store: `useLayoutAnalysis(getDocumentId)`

Document-scoped store that returns reactive plugin state and methods.

#### Parameters

| Parameter | Type | Description |
| :--- | :--- | :--- |
| **`getDocumentId`** | `() => string \| null` | A getter function that returns the document ID to track. |
...
| **`scope`** | `LayoutAnalysisScope \| null` | Document-scoped methods (`analyzePage`, `analyzeAllPages`, etc.). |
| **`provides`** | `LayoutAnalysisCapability` | Full plugin capability with global methods. |

### Store: `useLayoutAnalysisCapability()`

Low-level store that returns the full plugin capability directly.
