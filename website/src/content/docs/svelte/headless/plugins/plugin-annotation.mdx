...
```typescript
annotation.provides?.exportAnnotations({ pageIndex: 0 }).wait(
  (exported) => { /* ... */ },
  (error) => { /* ... */ },
);
```
...
```typescript
annotation.provides?.importAnnotations(exported);
```
...
and honors them in the Svelte viewer:
...
them directly and drive your own UI from them. The Svelte store returns a
reactive object with getters — keep it whole and read through `annotation.provides`
rather than destructuring (which would snapshot the getter value and lose
reactivity):
...
<script lang="ts">
  ...
  let { documentId, id }: { documentId: string; id: string } = $props();
  const annotation = useAnnotation(() => documentId);

  const flags = $derived(
    annotation.provides?.getAnnotationById(id)?.object.flags ?? [],
  );
  const isLocked = $derived(flags.includes('locked'));
</script>
```
...
```ts
const current = annotation.provides?.getAnnotationById(id)?.object.flags ?? [];
const next = current.includes('locked')
  ? current.filter((f) => f !== 'locked')
  : [...current, 'locked'];

annotation.provides?.updateAnnotation(pageIndex, id, { flags: next });
```

Custom `selectionMenuSnippet` snippets also receive `structurallyLocked` and
`contentLocked` on the `context` argument so they can disable destructive or
content-editing menu items without reading the flags array themselves. The same
two booleans are exposed as `isAnnotationStructurallyLocked(annotation)` and
`isAnnotationContentLocked(annotation)` on the annotation capability if you need
them outside of a selection menu.
...
| **`exportAnnotations(..)`** | Exports annotations as `AnnotationTransferItem[]` (stamps include `ctx.data`). |
| **`importAnnotations(..)`**| Imports an array of annotations into the viewer. |
| **`deleteAllAnnotations()`** | Removes every annotation from the current document. |
