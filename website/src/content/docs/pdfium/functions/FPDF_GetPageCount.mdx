...
async function getPdfInfo(pdfData: Uint8Array) {
  // Initialize PDFium
  ...
  try {
    ...
    // You can use the page count to iterate through all pages
...
fetch('sample.pdf')
  ...
  .then(buffer => getPdfInfo(new Uint8Array(buffer)))
  .then(info => console.log(`Document processed. It has ${info.pageCount} pages.`))
