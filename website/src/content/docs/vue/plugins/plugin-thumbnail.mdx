...
To handle navigation, you can use the `useScrollSvelte` composable to get the `scrollToPage` function and the `currentPage` for highlighting the active thumbnail.
...
const { state, provides } = useScrollSvelte();
</script>
