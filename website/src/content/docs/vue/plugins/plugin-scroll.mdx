...
The Scroll Plugin is a foundational plugin responsible for page layout, virtualization, and navigation. It provides the essential `<Scroller />` component, which intelligently renders only the visible pages to ensure high performance, and the `useScrollSvelte` composable for building page navigation controls.
...
The plugin provides the `<Scroller />` component for rendering pages and the `useScrollSvelte` composable for navigation.
...
The `useScrollSvelte` composable provides everything you need to build a page navigation UI. It returns a `provides` object with action methods and a reactive `state` object with data like the current page number.

```vue
<script setup lang="ts">
import { useScrollSvelte } from '@embedpdf/plugin-scroll/vue';

const { provides: scroll, state } = useScrollSvelte();
</script>
