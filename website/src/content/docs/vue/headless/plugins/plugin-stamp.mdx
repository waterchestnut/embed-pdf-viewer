---
title: Stamp Plugin
description: Add reusable rubber-stamp libraries in Vue, place built-in stamps, and let users turn annotations into custom stamps.
searchable: true
---
...
const stamps = useStampsByLibrary(() => selectedLibraryId.value, () => 'sidebar');
...
<template>
  ...
  >
    ...
    <span v-if="activeStamp?.libraryId === library.id && activeStamp?.stamp.id === stamp.id">
      placing
    </span>
  </button>
</template>
...
import { useStampCapability } from '@embedpdf/plugin-stamp/vue';
...
const selectedAnnotation = computed(() => {
  annotation.state.value.selectedUid;
  return annotation.provides.value?.getSelectedAnnotation() ?? null;
});

const handleCreateStamp = () => {
  if (!selectedAnnotation.value || !stampCapability.value) return;
...
const handleExport = () => {
  stampCapability.value?.exportLibrary('custom').wait((exported) => {
...
<CodeExample
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/stamp-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/stamp-example-content.vue"
  ]}
>
  <StampExample />
</CodeExample>
...
```vue
stampCapability.value?.loadLibrary({
...
| :--- | :--- | :--- |
| **`provides`** | `Ref<StampCapability \| null>` | The stamp API object, or `null` until the plugin is ready. |
...
| Property | Type | Description |
| :--- | :--- | :--- |
| **`libraries`** | `Ref<StampLibrary[]>` | The loaded libraries, including manifest-backed and runtime-created libraries. |
...
| :--- | :--- | :--- |
| **`libraryId`** | `MaybeRefOrGetter<string \| undefined>` | The library ID to filter by. Use `'all'` or `undefined` to include every library. |
| **`category`** | `MaybeRefOrGetter<string \| undefined>` | Optional category filter applied to both library-level and stamp-level categories. |
