...
```typescript
annotationApi.value?.exportAnnotations({ pageIndex: 0 }).wait(
  (exported) => { /* ... */ },
  (error) => { /* ... */ },
);
```
...
```typescript
annotationApi.value?.importAnnotations(exported);
```
...
annotationApi.value?.importAnnotations([
...
import { ImportExportExample } from '../../code-examples/headless/annotation-import-export-example';

<CodeExample
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/annotation-import-export-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/annotation-import-export-example-content.vue"
  ]}
>
  <ImportExportExample />
</CodeExample>
...
and honors them in the Vue viewer:
...
const props = defineProps<{ documentId: string; id: string }>();
const { provides: annotationApi } = useAnnotation(() => props.documentId);

const annotation = computed(
  () => annotationApi.value?.getAnnotationById(props.id)?.object ?? null,
);
const isLocked = computed(() => annotation.value?.flags?.includes('locked') ?? false);
</script>
```
...
```ts
const current = annotation.value?.flags ?? [];
const next = current.includes('locked')
  ? current.filter((f) => f !== 'locked')
  : [...current, 'locked'];

annotationApi.value?.updateAnnotation(pageIndex, id, { flags: next });
```

Custom `#selection-menu` slots also receive `structurallyLocked` and `contentLocked`
on the `context` argument so they can disable destructive or content-editing items
...
import { AnnotationFlagsExample } from '../../code-examples/headless/annotation-flags-example';

<CodeExample
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/annotation-flags-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/annotation-flags-example-content.vue"
  ]}
>
  <AnnotationFlagsExample />
</CodeExample>
