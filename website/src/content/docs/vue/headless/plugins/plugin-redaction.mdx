  ...
  <Scroller :document-id="activeDocumentId">
    <template #default="{ page }">
      <PagePointerProvider :document-id="activeDocumentId" :page-index="page.pageIndex">
        ...
        <AnnotationLayer :document-id="activeDocumentId" :page-index="page.pageIndex" />
...
```vue
<script setup lang="ts">
import { useRedaction, RedactionMode } from '@embedpdf/plugin-redaction/vue';

const props = defineProps<{ documentId: string }>();
const { state, provides } = useRedaction(() => props.documentId);

// Toggle unified redact mode (supports both text and area)
const handleToggle = () => provides.value?.toggleRedact();

// Check if redact mode is active
const isActive = computed(() => state.value.activeType === RedactionMode.Redact);
</script>
```
...
In annotation mode, redaction colors can be customized. Use the `useAnnotation` composable to access the annotation state and update colors on selected annotations:
...
const changeColor = (color: string) => {
  if (!selectedRedact.value || !annotationApi.value) return;
...
import { RedactionAnnotationExample } from '../../code-examples/headless/redaction-annotation-example';

<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/redaction-annotation-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/redaction-annotation-example-content.vue"
  ]}
>
  <RedactionAnnotationExample />
</CodeExample>
