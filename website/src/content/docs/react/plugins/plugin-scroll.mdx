...
The Scroll Plugin is a foundational plugin responsible for page layout, virtualization, and navigation. It provides the essential `<Scroller />` component, which intelligently renders only the visible pages to ensure high performance, and the `useScrollSvelte` hook for building page navigation controls.
...
The plugin provides the `<Scroller />` component for rendering pages and the `useScrollSvelte` hook for navigation.
...
The `useScrollSvelte` hook provides everything you need to build a page navigation UI. It returns a `provides` object with action methods and a `state` object with reactive data like the current page number.

```tsx {1,4, 8-9}
import { useScrollSvelte } from '@embedpdf/plugin-scroll/react';
